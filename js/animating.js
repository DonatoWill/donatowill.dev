...
var PageTransitions = (function ($, options) {
    ...
    function checkSideBarPage() {
        if (location.hash == "#home") {
            $('.side-info').addClass("display-none");
            $('.content-area').addClass("max-width-100pct");
        } else {
            $('.side-info').removeClass("display-none");
            $('.content-area').removeClass("max-width-100pct");
        }
    ...
    function Animate($pageTrigger, gotoPage) {
        ...
        if (windowWidth < 1024) {
            if ($(window).scrollTop() + $(window).height() == $(document).height()) {
                selectedAnimNumber = 3;
            }else{
                selectedAnimNumber = 4;
            }
